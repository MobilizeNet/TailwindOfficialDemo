...
namespace TailwindPOS
{
   ...
   internal static class MainModule
   {
      ...
      [Intercepted]
      static string URL_PRODUCT_API { get; set; } = "";

      // The Cash Drawer must have a minimum amount in order to start a Shift
      [Intercepted]
      static int MINIMUM_CASH { get; set; } = 0;
      ...
      const int SXH_SERVER_CERT_IGNORE_ALL_SERVER_ERRORS = 13056;

      internal static int CreateTicket(int CustomerID, int POSID, int UserID)
      {
         System.DateTime currentDate = DateTime.Parse(DateTimeHelper.ToString(DateTime.Now));
         ...
         command.CommandText = "Insert into Tickets(Ticket_Datetime, Total, Taxes, CustomerID,UserID,POSID, Status) " + " values (@TicketDateTime, 0, 0, @CustomerID, @UserID, @POSID, 1)";
         DbParameter TempParameter = null;
         TempParameter = command.CreateParameter();
         TempParameter.ParameterName = "@TicketDateTime";
         TempParameter.DbType = DbType.DateTime;
         TempParameter.Direction = ParameterDirection.Input;
         TempParameter.Size = 0;
         TempParameter.Value = currentDate;
      ...
      internal static void CompleteTicket(DbConnection conn, decimal Total, decimal taxes, int POSID, int UserID)
      {
         ...
         command.Connection = conn;
         UpgradeHelpers.DB.DbConnectionHelper.ResetCommandTimeOut(command);
         command.CommandText = "Update Tickets set Total = @Total, Taxes = @Taxes, Status = 0 " + " Where POSID = @POSID and TicketID = @TicketID";
         DbParameter TempParameter = null;
         TempParameter = command.CreateParameter();
      ...
      internal static void EndShiftWithAmount(int POSID, int UserID, ref System.DateTime ShiftID, decimal StartCash)
      {
         object EndCash = null;
         object Endshift = null;
         object StartShift = null;
         ShiftID = DateTime.Parse(DateTimeHelper.ToString(ShiftID));
         DbConnection conn = OpenConnection();
         // We need to insert the shift information
         DbCommand command = UpgradeHelpers.DB.AdoFactoryManager.GetFactory().CreateCommand();
         command.Connection = conn;
         UpgradeHelpers.DB.DbConnectionHelper.ResetCommandTimeOut(command);
         command.CommandText = "Update Shifts set EndShift = @EndShift, EndCash = @EndCash) " + " where POSID = @POSID and UserID = @UserID and ShiftID = @ShiftID";
         DbParameter TempParameter = null;
         TempParameter = command.CreateParameter();
         ...
         TempParameter_4.DbType = DbType.DateTime;
         TempParameter_4.Direction = ParameterDirection.Input;
         TempParameter_4.Size = 0;
      ...
      {
         string query = "select * from Customers where LastName = @keyword " + "Union  " + "Select * from Customers where FirstName = @keyword  " + "Union " + "Select * from Customers where email = @keyword  ";
         DbConnection conn = OpenConnection();
      ...
      internal static bool FindProductByCode(string productCode, ref string ProductDescription, ref decimal ProductPrice)
      {
         ...
         if ( xmlhttp.status == 200 )
         {
            ...
            {
               result = false;
               Mobilize.Web.MessageBox.Show("Product not found", AssemblyHelper.GetTitle(System.Reflection.Assembly.GetExecutingAssembly()));
               return result;
            }
      ...
      public static void Main()
      {
         ...
         {
            foreach ( POSSystems POSIterator in POSSystems.Values )
            {
               POS = POSIterator;
      ...
      internal static void RegisterBreakEnd(int POSID, int ShiftID, int BreakId, ref System.DateTime EndTime)
      {
         ...
         EndTime = DateTime.Parse(DateTimeHelper.ToString(EndTime));
         DbConnection conn = null;
         conn.ConnectionString = OpenConnection().ConnectionString;
         DbCommand objcommand = UpgradeHelpers.DB.AdoFactoryManager.GetFactory().CreateCommand();
         objcommand.Connection = conn;
         UpgradeHelpers.DB.DbConnectionHelper.ResetCommandTimeOut(objcommand);
         objcommand.CommandText = "UPDATE Breaks Set EndTime = @EndTime where" + " POSID = @POSID, ShiftID = @ShiftID, BreakID = @BreakID";
         DbParameter TempParameter = null;
         TempParameter = objcommand.CreateParameter();
         TempParameter.ParameterName = "@POSID";
         TempParameter.DbType = DbType.Decimal;
      ...
      internal static int RegisterBreakStart(int POSID, int ShiftID, ref System.DateTime StartTime)
      {
         ...
         objcommand.Connection = conn;
         UpgradeHelpers.DB.DbConnectionHelper.ResetCommandTimeOut(objcommand);
         objcommand.CommandText = "Insert into Breaks(POSID,ShiftID, StartTime) " + " values (@POSID, @ShiftID, @StartTime)";
         DbParameter TempParameter = null;
         TempParameter = objcommand.CreateParameter();
         TempParameter.ParameterName = "@POSID";
         TempParameter.DbType = DbType.Decimal;
         ...
         TempParameter_3.DbType = DbType.DateTime;
         TempParameter_3.Direction = ParameterDirection.Input;
         TempParameter_3.Size = 0;
      ...
      internal static double SaveNewCustomer(string FirstName, string LastName, string Email, string Company, string Phone, string StreetAddress1, string StreetAddress2, string State, string City, string ZipCode, string County)
      {
         ...
         objcommand.Connection = conn;
         UpgradeHelpers.DB.DbConnectionHelper.ResetCommandTimeOut(objcommand);
         objcommand.CommandText = "Insert into Customers(FirstName, LastName, Email, Company, Phone, StreetAddress1, StreetAddress2, State, City, ZipCode, County) " + " values (@FirstName, @LastName, @Email, @Company, @Phone, @StreetAddress1, @StreetAddress2, @State, @City, @ZipCode, @County)";
         DbParameter TempParameter = null;
         TempParameter = objcommand.CreateParameter();
      ...
      internal static bool SaveSale(decimal m_SubTotal, decimal taxes, decimal Total, OrderedDictionary payments, OrderedDictionary items, int currentCustomer, int POSID, int UserID)
      {
         ...
         try
         {
            ...
            foreach ( Payment payIterator in payments.Values )
            {
               pay = payIterator;
               currentPayment = pay;
            ...
            foreach ( TicketItem itemIterator in items.Values )
            {
               item = itemIterator;
               currentItem = item;
               SaveItem(conn, CurrentTicketID, POSID, currentItem);
               item = default(TicketItem);
            }
      ...
      internal static int StartShiftWithAmount(int POSID, int UserID, ref System.DateTime StartShift, decimal StartCash)
      {
         ...
         command.Connection = conn;
         UpgradeHelpers.DB.DbConnectionHelper.ResetCommandTimeOut(command);
         command.CommandText = "Insert into Shifts(POSID, UserID, StartShift, StartCash) " + " values (@POSID, @UserID, @StartShift, @StartCash)";
         DbParameter TempParameter = null;
         TempParameter = command.CreateParameter();
         TempParameter.ParameterName = "@POSID";
         ...
         TempParameter_4.DbType = DbType.Currency;
         TempParameter_4.Direction = ParameterDirection.Input;
         TempParameter_4.Size = 0;
